...
def main():
    ...
    required_attributes = ["yellow"]

    required_embedding = [1 if attr in required_attributes else 0 for attr in attributes]
    print(f"\nEmbedding for required attribute{required_attributes}:{required_embedding}\n")


    if required_fruit in embeddings:
        required_fruit = embeddings[required_fruit]
    ...
    found_fruits = finding_fruit(embeddings, required_embedding)
    if found_fruits:
